import "./DetectedFlowers.css";

function DetectedFlowers({ onBack, onGoResult }) {
  return (
    <main className="detected-page">
      <button className="detected-back-button" onClick={onBack}>
        &lt;
      </button>

      <h1 className="detected-title">인식 결과</h1>

      <section className="detected-image-box">
        <div className="flower-box rose">
          <span>장미</span>
        </div>
        <div className="flower-box tulip">
          <span>튤립</span>
        </div>
        <div className="flower-box baby">
          <span>안개꽃</span>
        </div>
      </section>

      <p className="detected-section-title">인식된 꽃</p>
      <div className="detected-line" />

      <section className="detected-list">
        <div className="detected-item">
          <div className="item-bar" />
          <div>
            <h2>빨간 장미</h2>
            <p>신뢰도 98%</p>
          </div>
        </div>

        <div className="detected-item">
          <div className="item-bar" />
          <div>
            <h2>튤립</h2>
            <p>신뢰도 95%</p>
          </div>
        </div>

        <div className="detected-item">
          <div className="item-bar" />
          <div>
            <h2>안개꽃</h2>
            <p>신뢰도 92%</p>
          </div>
        </div>
      </section>

      <button className="flower-meaning-button" onClick={onGoResult}>
        꽃말 확인하기
      </button>

      <button className="retry-button" onClick={onBack}>
        인식이 잘못되었나요? 다시 촬영해보세요
      </button>
    </main>
  );
}

export default DetectedFlowers;